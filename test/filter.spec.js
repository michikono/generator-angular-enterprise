...
describe("filter generation", function() {
  ...
  it('creates filter files', function (done) {
    ...
    .on('end', function() {
      assert.file([
        'client/app/filters/my-filter/my-filter.filter.js',
        'client/app/filters/my-filter/my-filter.filter.spec.js'
      ]);
      done();
    });
  });
  ...
  it('generates expected filter content', function(done) {
    filterGen.run()
    .on('end', function() {
      var generatedfilterContent = fs.readFileSync(filterGen.destinationPath('client/app/filters/my-filter/my-filter.filter.js'), 'utf8');
